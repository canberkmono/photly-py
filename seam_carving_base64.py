...
def readb64(base64_string, color_code=1):
    msg = base64.b64decode(base64_string)
    sbuf = io.BytesIO(msg)
    if color_code == 0:
        pimg = Image.open(sbuf).convert('L')
        output = np.array(pimg)
    else:
        pimg = Image.open(sbuf).convert('RGB')
        output = cv2.cvtColor(np.array(pimg), cv2.COLOR_RGB2BGR)
...
def get_minimum_seam(im, mask=None, remove_mask=None):
    ...
    M_aug_2 = np.copy(M)
    # idx_mat = np.zeros_like(M, dtype=np.int)
    #
    # populate DP matrix
    for i in range(1, h):
        for j in range(0, w):
...
def prepare_final_image(img, rmask, output):
    img = img / 255.0
    rmask = np.expand_dims(rmask,2) / 255.0
    output = output / 255.0
    new_out = img * (1 - rmask) + output * rmask

    return np.uint8(255 * new_out)

def remove_object_base64(im_base64, rmask_64):
    im = readb64(im_base64)
    assert im is not None
    # mask = readb64(mask_base64, 0) if mask_base64 else None
    rmask = readb64(rmask_64, 0) if rmask_64 else None

    assert rmask is not None, "rmask cannot be None, please define a valid rmask"
    output = object_removal(im, rmask, None, False, False)
    output = prepare_final_image(im, rmask, output)
    output = opencv_to_base64(output)
...
if __name__ == '__main__':
    ...
    im = opencv_to_base64(im)
    rmask = opencv_to_base64(rmask)
    out_base64 = remove_object_base64(im, rmask)
    output = readb64(out_base64)
    # output = opencv_to_base64(output)
    # print("mask shape is {} and {}".format(np.max(output), np.max(rmask)))
    cv2.imwrite(OUTPUT_NAME, output)
